<template>
  ...
  >
    ...
    <form class="add-brand-form row" @submit.prevent="submitAddBrand">
...
import { addBrand } from "@/services/Admin/Products";
...
export default {
  ...
  methods: {
    ...
    async submitAddBrand() {
      this.isLoading = true;
      let brandArray = [];
      brandArray.push(this.brandForm);
      try {
        await addBrand(brandArray);
        this.$emit("success");
      } catch (error) {
        console.error(error);
