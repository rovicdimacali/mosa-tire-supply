<template>
  <div class="pickup row">
    <div class="pickup-copy col">
      <h1>🚗 Order Now, Pickup Later: Your Tire, Your Time!</h1>
      <p>
        Life moves fast, and we get it. That's why we've made getting new tires
        a breeze with our revolutionary service. Experience the ultimate
        convenience – choose the perfect set of tires online, and we'll have
        them ready for you when you arrive!
      </p>
      <Button label="Order Now and Skip the Queue" class="order-now" />
    </div>
    <img
      src="https://res.cloudinary.com/dpm5vdakr/image/upload/v1709360647/mosawebapp/assets/Untitled_design_2_fgucjo.png"
      alt=""
    />
  </div>
</template>

<script>
export default {};
</script>

<style></style>
